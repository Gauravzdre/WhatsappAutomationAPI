...
-- Create templates table if it doesn't exist (simplified version for strategy integration)
...
CREATE TABLE IF NOT EXISTS content_pillar_analytics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  strategy_id UUID REFERENCES strategies(id) ON DELETE CASCADE,
...
-- Now enhance the templates table with strategy columns
DO $$ 
BEGIN
...
BEGIN
  ...
  -- Strategy policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own strategies') THEN
    CREATE POLICY "Users can manage their own strategies" ON strategies
      FOR ALL USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their strategy segments') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their content pillar analytics') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their strategy analytics') THEN
