-- =============================================================================
-- TEAM COLLABORATION FEATURES - DATABASE SCHEMA
-- =============================================================================
...
-- Teams policies
CREATE POLICY IF NOT EXISTS "Team owners can manage their teams" ON public.teams
    FOR ALL USING (auth.uid() = owner_id);

CREATE POLICY IF NOT EXISTS "Team members can view their teams" ON public.teams
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can view their team memberships" ON public.team_members
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Team owners and admins can manage members" ON public.team_members
    FOR ALL USING (
        EXISTS (
...
CREATE POLICY IF NOT EXISTS "Team members can view invitations for their teams" ON public.team_invitations
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.team_members 
...
CREATE POLICY IF NOT EXISTS "Team owners and admins can manage invitations" ON public.team_invitations
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.team_members tm
...
CREATE POLICY IF NOT EXISTS "Team members can view activity for their teams" ON public.team_activity_log
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.team_members 
...
-- Add team_id to ai_agents table
ALTER TABLE public.ai_agents ADD COLUMN IF NOT EXISTS team_id UUID REFERENCES public.teams(id) ON DELETE SET NULL;

-- Add team_id to schedules table
ALTER TABLE public.schedules ADD COLUMN IF NOT EXISTS team_id UUID REFERENCES public.teams(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_team_activity_log_team_id ON public.team_activity_log(team_id);
CREATE INDEX IF NOT EXISTS idx_team_activity_log_user_id ON public.team_activity_log(user_id);
CREATE INDEX IF NOT EXISTS idx_team_activity_log_created_at ON public.team_activity_log(created_at);

-- Add indexes for team_id columns in existing tables
...
CREATE OR REPLACE TRIGGER update_team_members_updated_at BEFORE UPDATE ON public.team_members
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE OR REPLACE TRIGGER update_team_invitations_updated_at BEFORE UPDATE ON public.team_invitations
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Trigger to create default team when profile is created
CREATE OR REPLACE TRIGGER create_default_team_trigger
